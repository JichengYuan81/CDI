...
def base_train(model, trainloader, criterion, optimizer, scheduler, epoch, transform, args):
    ...
    if epoch == args.epochs_base - 1 and len(style_features_list) > 0:
        ...
        print(f"The base class style features have been saved to: {style_features_path}")
        print(f"Dimensions of stylistic features preserved: {all_style_features.shape}")
...
def update_fc_ft(trainloader, data_transform, model, m, session, args):
    ...
    if os.path.exists(style_features_path):
        try:
            ...
            print(f"Base class style features have been successfully loaded: {base_style_features.shape}")
        except Exception as e:
            print(f"Failed to load base class style features: {e}")
            base_style_features = None
    else:
        print(f"The base class style sheet does not exist: {style_features_path}")
    ...
    with torch.enable_grad():
        for epoch in range(args.epochs_new):
            ...
            new_class_features = causal_outputs['class_features']  # 新类的类别特征
            ...
            if base_style_features is not None and new_class_features.size(0) > 0:
                ...
                # expanded_class_features: [batch_size * multiplier, class_dim]
                ...
                reconstructed_features = model.module.causal_module.decode(
                    expanded_class_features,    # [batch_size * multiplier, class_dim]
                    sampled_style_features     # [batch_size * multiplier, style_dim]
                )
